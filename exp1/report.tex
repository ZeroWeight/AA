...
经过进一步的计算，可以得到$R_o=3.3\rm{k}$$\Omega$,$r_{be}=100\Omega+\frac{26\rm{mV}}{4.65\rm{\mu A}}=5.7k$$\Omega,R_i=r_{be}//R_{b2}//R_{b1}=3.9k$$\Omega,\dot{A}_u=\frac{\beta(R_o//R_L)}{r_{be}}\frac{R_i}{R_i+R_s}=49.95$发现和前文的数量级估算是符合的。
\subsubsection{$I_{CQ}=2\rm{mA}$时}
此时同上节重新计算，得到$I_{BQ}=9.30\rm{\mu A},I_{EQ}=2\rm{mA},U_{EQ}=2.4\rm{V},U_{BQ}=3.1\rm{V},R_{b1}=43\rm{k}$$\Omega,R_W=23\rm{k}$$\Omega,R_o=3.3\rm{k}$$\Omega$,$r_{be}=2.9k$$\Omega,R_i=2.3k$$\Omega,\dot{A}_u=79.43,U_{CQ}=5.4\rm{V}$发现和前文的数量级估算是符合的。
\subsection{仿真测试}
...
\hline
1.00&50.06&3.88&572.879&3.2&0.04603&0.1346&24269.3\\
\hline
2.00&76.63&2.19&870.245&3.095&0.02686&0.25629&17592.4\\
\hline
\tabincell{c}{2.00\\ （射极电阻改变）}&7.344&9.87&85.379&16.72&0.00052&0.0193254&22272\\
\hline
\end{tabular}
